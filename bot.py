...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE, custom_text=None):
    ...
    await update.message.reply_text(
        "👇 Please use the menu buttons" if lang == 'en' else "👇 የሜኑ ቁልፎችን ይጠቀሙ"
    )
...
# ✅ change_lang MUST be defined before app.add_handler uses it
...
async def handle_web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🎉 Congratulations! Your bingo result has been recorded!\n\nData: {data}"
    )
